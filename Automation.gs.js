...
const ACTIVE = 'Active';
const AUTOMATION_FIELDS = ['Active','Date','Day','Hour','Run Schedule?','Sheets to Hide','Sheets to Show','Sheets to Clear','Days to Clear','Last run'];
...
function createTimerSheet () {
 let sheet = setupConfigSheet(
    AUTOMATION_SHEET,
    AUTOMATION_FIELDS, true
  );  
  sheet.appendRow(AUTOMATION_DOCS);
  sheet.getRange('B3:C').setDataValidation(
    SpreadsheetApp.newDataValidation().requireNumberGreaterThanOrEqualTo(0).build()
  );  
  sheet.getRange('A3:A').setDataValidation(
    SpreadsheetApp.newDataValidation().requireCheckbox().build()
  );
  sheet.getRange('D3:D').setDataValidation(
    SpreadsheetApp.newDataValidation().requireCheckbox().build()
  );
...
function clearTimers () {
  ScriptApp.getScriptTriggers().forEach(
    (trigger)=>{
...
function runAutomations () {
  ...
  for (let row of automationData) {
    ...
    if (row.Date) {
      ...
      if (now.getDate() == dateObject.getDate() && now.getMonth() == dateObject.getMonth() && now.getFullYear()==dateObject.getFullYear()) {
        isTheDay = true;
...
function runAutomation (data) {
  ...
  if (lastRun && isToday(lastRun)) {
    ...
    try {
      ...
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(AUTOMATION_SHEET).getRange(`${LASTRUN_COL}${data.__row}`).setValue(new Date());
    } catch (err) {
      console.log('Ran into error:',err);      
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(AUTOMATION_SHEET).getRange(`${ERR_COL}${data.__row}`).setValue(JSON.stringify(err))
      throw err;
    }
  }
}
...
function clearSheets (sheetnames, days='ALL') {
 for (let sheetname of sheetnames.filter(isName)) {
    ...
    if (sheet) {
      ...
      SpreadsheetApp.getUi().alert(`Did not find sheet named "${sheetname}" to clear. Sheet is listed in automation configuration on sheet ${AUTOMATION_SHEET}`);
    }
  }
}
...
function showSheets (sheetnames) {
  for (let sheetname of sheetnames.filter(isName)) {
    ...
    if (sheet) {
      ...
      SpreadsheetApp.getUi().alert(`Did not find sheet named "${sheetname}" to show. Sheet is listed in automation configuration on sheet ${AUTOMATION_SHEET}`);
    }
  }
}
...
function hideSheets (sheetnames) {
  for (let sheetname of sheetnames.filter(isName)) {
    ...
    if (sheet) {
      ...
      SpreadsheetApp.getUi().alert(`Did not find sheet named "${sheetname} to hide. Sheet is listed in automation configuration on sheet ${AUTOMATION_SHEET}`);
    }
  }
}
