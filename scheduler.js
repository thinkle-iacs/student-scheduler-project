...
function Scheduler () {
      ...
      for (let d of days) {
        if (row[d]) {
          ...
          if (student.schedule[d]) {
            console.log(`Ignoring request from ${sheetName} for ${request} on ${d}: student already scheduled.`)
            student.notes[d].push(`Unable to give request from ${sheetName} for ${request} on ${d}: student already scheduled.`);
          } else if (requestedBlock.Limit && requestedBlock.roster.length >= requestedBlock.Limit) {            
            student.notes[d].push(`Unable to give request from ${sheetName} for ${request} on ${d}: already at limit of ${requestedBlock.Limit}`);
          } else {
  ...
  this.writeOutput = function () {
    for (let d of days) {
      const FIELDS = ['Present',...STUDENT_FIELDS,...PLACEMENT_FIELDS,'Notes'];
      ...
      for (let s of this.students) {
        ...
        if (s.notes && s.notes[d]) {
          ...
          let notes = s.notes[d].filter((n)=>n.indexOf('Default Placement')==-1);
      ...
      sheet.getRange('A2:A').setDataValidation(SpreadsheetApp.newDataValidation().requireCheckbox().build())
      
      sheet.setFrozenRows(1);
      sheet.setFrozenColumns(1);
      let existingFilter = sheet.getDataRange().getFilter()
      if (existingFilter) {existingFilter.remove()}
      sheet.getDataRange().createFilter().sort(FIELDS.indexOf('Name')+1,true).sort(FIELDS.indexOf('Activity')+1,true)
...
function applyFormatting (sheet, FIELDS) {
  ...
  for (let column of WRAP) {    
    let index = FIELDS.indexOf(column) + 1;
    let range = sheet.getRange(1, index, sheet.getLastRow()-1,1);
    console.log('Set wrap on',sheet.getName(),'range',range.getFormula(),'with # of rows: ',range.getNumRows())
    range.setWrap(true);
  }
}
...
function createSchedule () {
  let ss = SpreadsheetApp.getActiveSpreadsheet();
  let placementSheets = ss.getSheetByName(PLACEMENT_SHEET_LIST).getDataRange().getValues();  
  placementSheets = placementSheets.slice(1)
  placementSheets.sort(
    (a,b)=>a[1]-b[1]
  )
