...
function createPlacementSheet () {
  let placementSheetName = SpreadsheetApp.getUi().prompt('Enter name for Placement Sheet (i.e. Student Placements)').getResponseText();
  ...
  let sheet = setupConfigSheet(
    placementSheetName,headers
  )  
  let placementList = ss.getSheetByName(PLACEMENT_SHEET_LIST);  
  placementList.appendRow(
    [
      placementSheetName,
      placementList.getLastRow()
    ]
  );
  ...
  for (let i=1; i<STUDENT_FIELDS.length; i++) {
    ...
    sheet.getRange(`${letter}2`)
    .setFormula(
      `arrayformula(iferror(vlookup($A2:$A,'${STUDENT_SHEET}'!A:${letter},${i+1},false),""))`
    );
